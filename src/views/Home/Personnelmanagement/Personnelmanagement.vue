<template>
  <div class="CarsManagement">
    <el-row class="header">
      <div class="search">
        <input type="text" placeholder="请输入人名或人员ID" class="searchInput" v-model="carQuery" />
    ...
    <el-main>
      <el-table
        ...
        size="mini"
      >
        <el-table-column align="center" prop="vehiclesTypeof" label="头像"></el-table-column>
        <el-table-column align="center" prop="vehiclesNumBering" label="姓名"></el-table-column>
        <el-table-column align="center" prop="vehiclesUseunit" label="性别"></el-table-column>
        <el-table-column align="center" prop="vehiclesNsumber" label="年龄"></el-table-column>
        <el-table-column align="center" prop="vehiclesNsumber" label="设备ID"></el-table-column>
        <el-table-column align="center" prop="vehiclesNsumber" label="联系电话"></el-table-column>
        <el-table-column align="center" prop="vehiclesNsumber" label="家庭住址"></el-table-column>
        <el-table-column align="center" label="紧急联系人1"></el-table-column>
        <el-table-column align="center" label="紧急联系人2"></el-table-column>
        <el-table-column align="center" label="身体状况">
          <template slot-scope="scope">
            <img
              src="@/assets/img/icon/修改IC.png"
              @click="handleEditorCar(scope.$index, scope.row)"
              class="editor_img"
              alt
            />
            <img
              src="@/assets/img/icon/删除IC.png"
    ...
    <change-car :type="type.add" v-if="isShowAddCar" @addCar="addCar"></change-car>
    <change-car
      :type="type.editor"
  ...
  methods: {
    ...
    addCar(bol, carInfo) {
      if (bol) {
        ...
        // 服务器添加车辆
        this.$http
          .post(`${config.httpBaseUrl}/vehucle/insertVehucle`, carInfo)
          .then(res => {
            if (res.code === 200) {
    ...
    editorCar(bol, carInfo) {
      ...
      if (bol) {
        ...
        // 服务器修改
        this.$http
          .post(`${config.httpBaseUrl}/vehucle/updateVehucle`, carInfo)
          .then(res => {
            if (res.code === 200) {
    ...
    handleDeleteCar(index, row) {
      this.$confirm(`此操作将永久删除该车辆, 是否继续?`, "提示", {
        ...
        .then(() => {
          // 服务器删除
          this.$http
            .get(`${config.httpBaseUrl}/vehucle/deleteVehucle`, {
              params: {
    ...
    handleSearch() {
      this.$http
        .get(`${config.httpBaseUrl}/vehucle/getVehucle`, {
          params: {
